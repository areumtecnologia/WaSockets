    ...
    if (protocolMsg) {
        ...
        if (pollMsg) {
            try {
                ...
                let pollCreatorJid = getKeyAuthor(pollMsg.key || creationMsgKey, meLidNormalised)
        ...
        if (eventMsg) {
            try {
                const meLidNormalised = jidNormalizedUser(meLid)
                const eventCreatorJid = getKeyAuthor(eventMsg.key || creationMsgKey, meLidNormalised)
                const responderJid = getKeyAuthor(message.key, meLidNormalised)
                const eventEncKey = eventMsg.messageContextInfo?.messageSecret
